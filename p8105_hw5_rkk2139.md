...
![](p8105_hw5_rkk2139_files/figure-gfm/unnamed-chunk-3-2.png)<!-- -->
...
![](p8105_hw5_rkk2139_files/figure-gfm/plot_power_vs_true_mean-1.png)<!-- -->
...
ggplot(summary_results, aes(x = true_mu)) +
  ...
  geom_point(aes(y = avg_mu_hat, color = "All Samples"), size = 2) +
  geom_line(aes(y = avg_mu_hat_rejected, color = "Samples with Null Rejected")) +
  geom_point(aes(y = avg_mu_hat_rejected, color = "Samples with Null Rejected"), size = 2) +
  labs(title = "Average Estimate of µ̂ vs. True µ",
       x = "True µ",
       y = "Average Estimate of µ̂",
       color = "Legend") +
  theme_minimal() +
  scale_color_manual(values = c("All Samples" = "blue", "Samples with Null Rejected" = "red"))
```

![](p8105_hw5_rkk2139_files/figure-gfm/plot_avg_mu_hat-1.png)<!-- -->
...
Therefore the sample average of μ^ across tests for which the null is
rejected is NOT approximately equal to the true value of μ. In cases
where the null hypothesis is rejected, the estimated μ^ is more likely,
on average, to be further from zero than those estimates that do not
lead to rejection. This creates a selection bias where only larger
...
**victim_last** and **victim_first**: Name of the homicide victim.

**victim_race**: Victim’s race.

**victim_age**: Age of the victim.

**victim_sex**: Victim’s sex.
...
homicide_data <- homicide_data %>%
  # Create city_state variable by combining city and state in table
...
``` r
# Standardize disposition values (avoid case and whitespace issues)
homicide_data <- homicide_data %>%
  mutate(
...
nrow(baltimore_unsolved)  # Check the number of rows
```
...
![](p8105_hw5_rkk2139_files/figure-gfm/estimate_plot-1.png)<!-- -->
